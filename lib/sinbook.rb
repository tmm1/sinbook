...
class Sinbook
  def initialize opts
    [ :api_key, :secret ].each do |var|
      raise ArgumentError, "required option #{var} missing" unless opts[var]
      instance_variable_set("@#{var}", opts[var])
    end

    [ :app_id, :url, :callback, :symbolize_keys ].each do |var|
      instance_variable_set("@#{var}", opts[var]) if opts.has_key?(var)
    end
  end
  ...
  def require_login!
    raise ArgumentError, 'no response supplied' unless response
    raise ArgumentError, 'app url was not set' unless self.url

    if valid?
      self.redirect addurl unless params[:user]
    else
  ...
  def redirect url
    ...
    if params[:in_iframe]
      response.body "<script type='text/javascript'>top.location.href=#{url.dump}</script>"
    else
      response.body "<fb:redirect url=#{url.dump}/>"
    end
  end
  ...
  def params
    ...
    request.env['facebook.params'] ||= \
      request.env['facebook.vars'].inject({}) do |h,(k,v)|
        s = k.to_sym
        case k
        when 'friends'
  ...
  def valid?
    if request.nil?
      return false
    elsif request.params['fb_sig'] # canvas/iframe mode
      prefix = 'fb_sig'
      vars = request.POST[prefix] ? request.POST : request.GET
    elsif request.cookies[api_key] # fbconnect mode
      prefix = api_key
      vars = request.cookies
    else
      return false
    end

    if request.env['facebook.valid?'].nil?
      fbvars = {}
      ...
      if request.env['facebook.valid?'] = (vars[prefix] == sig)
        request.env['facebook.vars'] = fbvars
      end
    end

    request.env['facebook.valid?']
  end
